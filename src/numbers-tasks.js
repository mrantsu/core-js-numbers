...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengths = Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2);
  const cosAngle = dotProduct / lengths;
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const currentNumber = fibonacciList[i - 1] + fibonacciList[i - 2];
    fibonacciList.push(currentNumber);
  }
...
function getSumOfDigits(num) {
  ...
  return numToString.split('').reduce((acc, item) => +acc + +item);
}
